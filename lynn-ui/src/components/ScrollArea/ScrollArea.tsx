...
interface ScrollAreaCSSProperties extends CSSProperties {
  '--lynn-scroll-radius'?: string;
}
...
export function ScrollArea(props: ScrollAreaProps) {
  ...
  const { rootRef, viewportRef, thumbRef } = useCustomScrollbar<
    HTMLDivElement,
    HTMLDivElement,
    SVGPathElement
  >({ orientation, idleDelay, geometry: 'path' });

  const rootStyle: ScrollAreaCSSProperties = {
  ...
  return (
    ...
    >
      {/*
        `tabIndex={0}` is deliberate: a region that scrolls has to be
        reachable by keyboard, or its content is only readable with a mouse.
        The role/name pair is what stops that tab stop from being a silent one.
      */}
      <div
        ref={viewportRef}
        className="lynn-scroll-area-viewport"
        tabIndex={0}
        role="group"
        aria-label={ariaLabel}
      >
        {/*
          One wrapper the hook can hand to a `ResizeObserver`: content that
          grows or shrinks (a filter clearing, an image finishing) changes the
          thumb ratio without ever firing a scroll event.
        */}
        <div className="lynn-scroll-area-content">{children}</div>
      </div>
      {/*
        The curved track. No `viewBox`: the SVG is stretched over the root's
        padding box at 100% x 100%, so one user unit is one CSS px and the
        `d` the hook writes is plain px geometry of this element's own box -
        which is also what makes `getTotalLength()` return px of track.

        The hit path comes first so that `:hover` on it can reach the thumb
        path as a later sibling; it paints nothing, and only it opts back into
        pointer events, so the rest of the overlay stays click-through.
      */}
      <svg
        className="lynn-scroll-area-track"
        aria-hidden="true"
        focusable="false"
      >
        <path ref={thumbRef} className="lynn-scroll-area-hit" />
        <path className="lynn-scroll-area-rail" />
        <path className="lynn-scroll-area-thumb" />
      </svg>
    </div>
  );
}
