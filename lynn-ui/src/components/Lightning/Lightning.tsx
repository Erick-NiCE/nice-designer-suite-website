...
interface LightningCSSProperties extends CSSProperties {
  '--lynn-lightning-color'?: string;
  '--lynn-lightning-glow'?: string;
}
...
export function Lightning(props: LightningProps) {
  ...
  useEffect(() => {
    ...
    const edgePoint = (d: number, inset: number): Point => {
    ...
    const drawFrame = () => {
      ...
      for (let idx = 0; idx < tendrils.length; idx++) {
        ...
        stroke(
          points,
          1.2,
          rgba(brightRgb, 0.75 + Math.random() * 0.25),
          rgba(baseRgb, 0.95),
          8
        );
      }
      ...
      if (mctx) {
        ...
        for (const mote of motes) {
          const inset =
            mote.inset + Math.sin(t * mote.oscSpeed + mote.phase) * mote.osc;
          const p = edgePoint(mote.d + t * mote.speed, inset);
          const twinkle =
            mote.tw * (0.55 + 0.45 * Math.sin(t * 2.1 + mote.phase * 2));
          mctx.beginPath();
  ...
  const ringVars: LightningCSSProperties = {
  ...
  return (
    <div
      ref={containerRef}
      className={classes}
      style={{ ...style, ...ringVars }}
    >
      <div className="lynn-lightning-ring" aria-hidden="true">
        <div className="lynn-lightning-ring-inner" />
      </div>
      <canvas
        ref={plasmaRef}
        className="lynn-lightning-plasma"
        style={{ opacity }}
        aria-hidden="true"
      />
      {moteCount > 0 ? (
        <canvas
          ref={motesRef}
          className="lynn-lightning-motes"
          aria-hidden="true"
        />
      ) : null}
      {children != null ? (
        <div className="lynn-lightning-content">{children}</div>
      ) : null}
    </div>
  );
}
