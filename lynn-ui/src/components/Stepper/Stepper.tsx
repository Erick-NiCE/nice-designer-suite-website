  ...
  return (
    <ol className={classes} style={style}>
      {steps.map((step, index) => (
        <li
          className="lynn-stepper-step"
          key={step.id ?? index}
          id={step.id}
        >
          <div className="lynn-stepper-sidebar">
            <StepNumber n={step.number ?? index + 1} size="lg" color={color} />
            <span className="lynn-stepper-line" aria-hidden="true" />
          </div>
          <div className="lynn-stepper-body">
            <div className="lynn-stepper-header">
              <h3 className="lynn-stepper-title">{step.title}</h3>
            </div>
            <div className="lynn-stepper-card">{step.children}</div>
          </div>
        </li>
      ))}
    </ol>
